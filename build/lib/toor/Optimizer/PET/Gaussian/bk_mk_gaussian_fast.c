...
const float invert2timesigma_z_square, int dataset_number, int n, int m, int p, const float* crystal_pitch_XY, const float* crystal_pitch_Z,
...
//            z_distance = fabsf(z_distance);
            ...
            if (fabsf(z_distance) <= acceptable_z_distance)
            {
...
//                d2_normal = crystal_pitch_Z[e] * sqrt(a_normal[e] * a_normal[e] + b_normal[e] * b_normal[e] + c_normal[e] * c_normal[e]);
                if (fabsf(y_distance) <= acceptable_y_distance)
                {
//                {
//                    x_distance = a_cf[e] * a_temp + b_cf[e] * b_temp + c_cf[e] * c_temp - d_cf[e];
//                {
