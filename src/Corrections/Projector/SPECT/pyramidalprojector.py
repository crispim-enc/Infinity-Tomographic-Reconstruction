...
class PyramidalProjector:
    def __init__(self, voxelSize=None, FoVAxial=45, FoVRadial=25, FoVTangencial=30):
        """
        Pyramidal Projector
        ...
        self.FoVRadial_min = 23/voxelSize[0]
        self.FoVRadial_max = 40/voxelSize[0]
    ...
    def transformIntoPositivePoints(self):
        ...
        """
        for coor in range(3):
            abs_min_min = np.abs(np.min([np.min(self.pointCorner1List[:, coor]), np.min(self.pointCorner2List[:, coor]),
    ...
    def createVectorialSpace(self):
        ...
        # self.x_range_lim = [self.pointCenterList[:,0].min(), self.pointCenterList[:,0].max()]
        # self.x_range_lim = [self.pointCenterList[:,0].min()+1/self.voxelSize[0], self.pointCenterList[:,0].max()+self.FoVRadial]
        self.x_range_lim = [self.pointCenterList[:,0].max()+self.FoVRadial_min,
                            self.pointCenterList[:,0].max()+self.FoVRadial_max]
        self.y_range_lim = [self.pointCenterList[:,1].min(), self.pointCenterList[:,1].max()]
        self.z_range_lim = [self.pointCenterList[:,2].min(), self.pointCenterList[:,2].max()]
    ...
    def createPlanes(self):
        ...
        self.aRight, self.bRight, self.cRight, cp = PyramidalProjector.crossProduct(v2, v3)
        self.dRight = PyramidalProjector.calcD(self.pointCenterList, cp)

        self.aFront, self.bFront, self.cFront, cp = PyramidalProjector.crossProduct(v3, v4)
