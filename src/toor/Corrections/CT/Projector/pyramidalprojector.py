...
class PyramidalProjector:
    def __init__(self, voxelSize=None, FoVAxial=45, FoVRadial=25, FoVTangencial=30, FovRadialStart=None,
        ...
        self._absMinMin = np.zeros((3,), dtype=np.float32)
    ...
    def setAbsMinMin(self):
        self._absMinMin = np.zeros((3,), dtype=np.float32)
        for coor in range(3):
            self._absMinMin[coor] = np.abs(np.min([np.min(self.pointCenterList[:, coor]),np.min(self.pointCorner1List[:, coor]), np.min(self.pointCorner2List[:, coor]),
    ...
    def transformIntoPositivePoints(self):
        ...
        for coor in range(3):
            ...
            #                     np.min(self.pointCorner3List[:, coor]), np.min(self.pointCorner4List[:, coor])]))
            abs_min_min = self._absMinMin[coor]
    ...
    def createVectorialSpace(self):
        ...
        self.min_x = self.pointCorner1List[:,0].min()
        self.min_y = self.pointCorner1List[:,1].min()
...
if __name__ == "__main__":
    from Geometry import HeadGeometry
