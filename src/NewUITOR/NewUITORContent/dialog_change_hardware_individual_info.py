...
class DialogChangeHardware(QtWidgets.QDialog,DialogChangeHardware_parameters,Ui_MainWindow_easypet_client):
    ...
    def _init_values_from_binary_file(self):
        ...
        self.baudrate = reading_hardware_parameters.baudrate
    ...
    def _get_data_from_dialog(self):
        ...
        real_size_crystal_dimensions_str = [0]*len(self.real_size_crystal_dimensions)
        for i in range(len(self.crystal_names_widgets)):
            ...
            try:
                ...
                real_size_crystal_dimensions_str[i] = str(self.real_size_crystal_dimensions[i])
        ...
        distance_between_motors_str = str(key_value[3])  # mm
        self.detectors_distances = [key_value[0], key_value[1],key_value[0]*key_value[2] , key_value[0]*(1-key_value[2])]
        detectors_distances_str = [str(i) for i in self.detectors_distances]
        start_index = 19
        end_index = start_index+len(real_size_crystal_dimensions_str)
        list_2_record[start_index:end_index] = real_size_crystal_dimensions_str
        start_index = end_index
        list_2_record[start_index] = distance_between_motors_str
        start_index += 1
        end_index = start_index + len(detectors_distances_str)
        list_2_record[start_index:end_index] = detectors_distances_str
    ...
    def _override_values(self):
        ...
        file_path = '{}/system_configurations/x_{}__y_{}'.format(directory, self.crystals_matrix[0], self.crystals_matrix[1])
        try:
